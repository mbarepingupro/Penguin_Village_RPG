...
function drawBuilding(id, bdef, level) {
    ...
    if (fontReady) {
        ...
        if (!cfg.noLevelBadge) {
            ...
            // (soft-cap cost curve, see building_cost() in app.py), no
            // ceiling -- always show the real level, never "★ MAX". (No
            // other building carrying a level badge can reach level 3+ in
            // the first place -- only these 5 ever increment current_level
            // past 1.)
